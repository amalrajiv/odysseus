...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Ariadne Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Ariadne kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Ariadne Chat';
    }
