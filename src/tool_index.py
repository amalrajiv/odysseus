...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_calendar": "Calendar event management. For 'upcoming events' / 'what's on my calendar' use action=list_events (optional start/end; defaults today + 14 days). For create/update/delete use create_event etc. in one call — pass calendar/calendar_id when the user names an account (Personal, Work); list_calendars only when the calendar is unknown. Required: summary + dtstart. All-day: all_day=true + YYYY-MM-DD. Timed: ISO datetime in user's local wall time. Event reminders: pass reminder_minutes (creates Notes reminder — do not also call manage_notes).",
