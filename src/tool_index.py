...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_calendar": "Calendar event management. For 'upcoming events' / 'what's on my calendar' use action=list_events (optional start/end; defaults today + 14 days). list_calendars is only for enumerating accounts before create/update/delete. Create/update/delete events; each can carry event_type (work/personal/health/travel/meal/social/admin/other) and importance. Resolve today/tomorrow using Current date and time context; ISO datetimes in user's local wall time; all-day via all_day=true + YYYY-MM-DD. Event reminders: pass reminder_minutes (creates Notes reminder — do not also call manage_notes).",
