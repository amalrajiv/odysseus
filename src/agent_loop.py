...
- Calendar READS ("upcoming events", "what's on my calendar", "this month", schedule, appointments): call `manage_calendar` with `action=list_events`. For "this month" / a named month pass `start` = first day and `end` = first day of next month (YYYY-MM-DD). Defaults to today + 14 days when omitted. After `list_events` succeeds, present the returned events grouped by date with their `[Summary](#event-<uid>)` links — do NOT re-call the tool, manually convert every timestamp, or re-plan. Do NOT use `action=list` or `list_calendars` for reads — `list_calendars` only lists calendar account names.
...
- Calendar READS: `manage_calendar` with `action=list_events` (for "this month" pass `start`/`end` as YYYY-MM-DD covering the full month). After it returns, format the events once — no re-calls, no per-event timezone math. Calendar WRITES: `create_event` / `update_event` / `delete_event` in one call — pass `calendar: "Personal"` (etc.) when the user names a calendar; only call `list_calendars` when the calendar is unknown. All-day: `all_day: true` + `dtstart` as YYYY-MM-DD.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Ariadne chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
```manage_calendar
{"action": "list_events", "start": "2026-07-01", "end": "2026-08-01"}
```
...
**Read schedule:** `action=list_events` with optional `start`/`end` (YYYY-MM-DD or ISO datetime). For "this month" use first day → first day of next month. Defaults today through +14 days. After it returns, present the events grouped by date — do NOT re-call or manually recompute every timestamp. `list_calendars` is ONLY for picking an account when the user did not name one — NOT for fetching events. \
...
```
GENERIC LOOPBACK to allowed Ariadne internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
