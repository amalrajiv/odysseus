...
_HF_TOKEN_STATUS_SNIPPET = (
    ...
    'Add one in Ariadne Cookbook -> Settings -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When Ariadne runs from a venv (e.g. native macOS install), put its bin
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python Ariadne is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            # Put Ariadne's own venv bin on PATH (local runs only) so the serve
            ...
            if not remote:
                ...
                if local_windows:
                    ...
                    # user PATH entries from the already-running Ariadne process.
            ...
            if ("llama_cpp" in req.cmd or "llama-server" in req.cmd) and not local_windows_llama_cmd:
                ...
                runner_lines.append('# Auto-generated by Ariadne Cookbook: a `llama-server` lookalike')
                ...
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[odysseus] WARNING: remote Ollama will bind to ${ODYSSEUS_OLLAMA_HOST}:${ODYSSEUS_OLLAMA_PORT} so Ariadne can reach it from this host."')
