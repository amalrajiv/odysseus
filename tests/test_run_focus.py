...
def test_dry_run_prints_command_and_does_not_execute(capsys):
    ...
    # run() prints shlex.join(command); build the expectation the same way so
    # the assertion holds even when sys.executable's path contains spaces.
...
def test_dry_run_last_failed_prints_safe_flags(capsys):
    ...
    assert executor.calls == []
    assert out == shlex.join(
        [sys.executable, "-m", "pytest", "--last-failed", "--last-failed-no-failures=none"]
    ) + "\n"
